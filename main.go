...
func main() {
	...
	json.Unmarshal(rawjson, &GameInfo)

	//robots := make(map[int]string)

	db = sqlhelper.New()
	db.Path = GameInfo.DBpath
	...
	file, err := os.ReadDir(GameInfo.Robotspath)
	if err != nil {
		log.Fatal(err)
	}
	...
	for _, f := range file {
		...
		tr.Loss = 0
		tr.Points = 0
		rc, err := os.ReadFile(GameInfo.Robotspath + "/" + robotFilename)
		if err != nil {
			log.Fatal(err)
		}
		tr.Code = string(rc)
		tr.CodeHash = hashing.StringHash(hashing.SHA256, tr.Code)
		sqli := InsertIntoTable("robots", tr)
		_, err = db.Db.Exec(sqli)
		if err != nil {
			log.Fatal(err)
		}
	...
	GameInfo.NumOfBots = 0
	sqlstring := "select count(*) as e from robots;"
	err = db.Db.QueryRow(sqlstring).Scan(&GameInfo.NumOfBots)
	if err != nil {
		log.Fatal(err)
	}

	GameInfo.RobotStorage = make([]therobots, 0)
	...
	for rows.Next() {
		...
		GameInfo.RobotStorage = append(GameInfo.RobotStorage, fn)
	}
	rows.Close()
	...
	// 			GameInfo.Robotspath+"/"+robotStorage[i].Filename,
	// 			GameInfo.Robotspath+"/"+robotStorage[j].Filename)
	// 		cmd.Stdout = buf
	// 		err := cmd.Run()
	// 		if err != nil {
